...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val adapter = PaperAdapter { item -> startActivity(Intent(this, DetailActivity::class.java).putExtra("item_id", item.id)) }
    ...
    private fun refreshList() {
        lifecycleScope.launch {
            val items = app.database.dao().getAllPapers()
            (binding.recyclerView.adapter as PaperAdapter).items = items
            (binding.recyclerView.adapter as PaperAdapter).notifyDataSetChanged()
